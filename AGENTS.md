...
- **The hooks are local-only.** They live in `.git/hooks/` (untracked), so they exist only where someone ran `graphify hook install`. A fresh clone, CI, or another machine won't auto-rebuild until the hook is installed there. See [`CONTRIBUTING.md`](CONTRIBUTING.md) for the one-time setup.
