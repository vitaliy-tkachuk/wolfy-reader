...
- **A minimal reset and nothing more** (`box-sizing`, zeroed body margin, `max-width:100%` on replaced content, and the containment caps below). The host must **not** put the body in a column context: layout containment forbids a contained box fragmenting across columns, which is why the paginator gives each chunk its own multi-column context. Column geometry belongs to the paginator; themes and typography to the appearance controls.
  - **Content wider than a reading column is capped by the reset, and two of those caps are `!important`** (2026-09-17). A page *is* a column and the chunk box is deliberately `overflow:visible` (a page turn reveals its later columns by translating it), so nothing clips a wide `<table>`, an unwrapped `<pre>` or a single unbreakable token — each paints over the neighbouring page's text. The reset adds `overflow-wrap:anywhere` on the content root, `pre{white-space:pre-wrap!important}` and `table{max-width:100%!important}`. Containment is page geometry rather than a formatting choice, and books do ship `pre{white-space:pre}` and `table{max-width:none}`, so those two win outright, by the same reasoning as the box-model pin below; `overflow-wrap` is left inheritable and overridable, and it is also what lets a capped table shrink (`max-width` alone leaves an auto-layout table at its min-content width). Proven in `test/browser/layout.browser.mjs` ("wide content stays inside its page"); see [`layout.md`](layout.md).
  - **The frame pins the html/body/root box model inline and `!important` at every relayout** (2026-09-17). The reset's `body{margin:0}` loses to any later or more specific publisher rule, and Gutenberg's stylesheet ships `@media screen{body{margin-left:10%;margin-right:10%}}`: the chunks are positioned inside the body box, so every page painted shifted right by the margin and clipped by the same amount on the right edge. Page geometry is the paginator's, so `relayout` sets `margin/padding/border` to zero and `min/max-width` open on `html`, `body` and the content root (plus `width/height:auto` on the two viewport boxes) as inline important declarations — the one thing no stylesheet outranks. The page-edge whitespace is the paginator's own `columnGap`, applied to the chunks, so nothing the reader wants is lost. Proven in `test/browser/layout.browser.mjs` ("publisher box model on html/body").
- **Section-invariant render inputs are cached per section; only the assembly re-runs on a reflow** (2026-08-26). Every appearance change and mode switch re-assembles the srcdoc (the theme rides it), and re-running the whole pipeline for that made a font-size tick re-decode bytes, re-sanitize, and re-base64 every image. `ContentHost` now caches the section-invariant inputs — the sanitized, resource-applied document, its serialized head, the assembled body markup (memoized per body key: plain / chunked@N), the `ResourceRegistry` with its minted `data:` URLs, and both summaries — keyed by the `Section` object. A re-render of the same section reuses them all; **the cache is invalidated when a different section renders and on `destroy()`** (the registry is released at exactly those two points, no longer per render). The security posture is untouched by design: everything cached sits strictly *downstream* of the sanitizer (the same sanitized output, byte for byte — sanitization is still unconditional), resources stay the same minted `data:` URLs (never `blob:`), the CSP nonce is still created fresh for every document assembly, and the frame stays `sandbox="allow-scripts"` opaque-origin. Proven in `test/browser/layout.browser.mjs` ("the render-input cache"): counted section/resource loads stay at 1 across an appearance tick and a mode switch, the image keeps the identical `data:` URL, and the CSP nonce differs per render.
...
- **`SVG_ELEMENTS` / `SVG_GLOBAL_ATTRIBUTES`** — a shape-and-paint subset. SVG is matched by *local* name so `xlink:href` and `href` share one rule. **SVG names are matched case-sensitively, HTML names case-insensitively** (2026-09-17): the tables spell SVG as the platform does (`viewBox`, `preserveAspectRatio`, `linearGradient`, `foreignObject`) and both parsers deliver those spellings — XML verbatim, the HTML parser through its SVG case-adjustment tables — so lowercasing before the lookup stripped every camelCase attribute (a Gutenberg cover lost its `viewBox` and rendered cropped) and unwrapped every camelCase element. The *discard* check alone is case-insensitive in both vocabularies, because the search extractor mirrors it over raw markup where it only sees lowercased tag names (`DISCARDED_SVG_ELEMENTS_LOWER`).
...
**`href` resolution prefers the format's own seam, then falls back to a heuristic** — and **degrades quietly on a miss** (no throw, stays put / page 0), consistent with the `Position` soft-miss philosophy. First choice: the current section's `Section.resolveHref?(href)` (core), which maps the section-relative href to a path-free `SectionRef` by the same authority the TOC uses (EPUB's resolved-path → section-id table). This is what makes cross-file chapter links work: `section.id` is an opaque manifest id (`item8`) that the href's file name (`..._1342-h-2.htm.xhtml`) never matches, so the id/filename heuristic alone silently soft-missed and clicking a chapter link did nothing. When the format supplies no `resolveHref` (or it misses), the facade falls back to the id/filename heuristic over public `Book` data, in order: (1) the path or its last segment equals a `section.id`; (2) the last segment, or that segment minus its extension, matches a `section.id` or an id's own last-segment/extension-less form; (3) a fragment the href carries is also carried by a `TocItem`, whose `sectionId` is then borrowed. A pure-fragment href (`#note`) seeks within the current section.
...
A text selection in the reader surfaces as a first-class `selection` event carrying the selected text, a `Position` whose quote spans the whole selection (anchored at its start, quoted to its full length), and the selection's geometry (`rect`, `rects`) so a host can anchor a popover on it. The event fires standalone — it reports, it never navigates — so it does not go through the navigation queue and emits no `positionchange`.
...
**Wire messages (frame→host, unsolicited, protocol v6; geometry v11; clearing v12):**

| message | payload | when |
|---|---|---|
| `selection` | `start`, `end`, `text`, `rect`, `rects` | a completed text selection in the frame — a non-collapsed selection with non-empty rects, settled on pointer/key/mouse release |
| `selectioncleared` | — | a release that settled a collapsed or empty selection while a `selection` from this document stood reported; once per reported selection |

`start`/`end` are UTF-16 code-unit offsets into the **tiled section text** (the same text space `sectionText` reports and the paginator measures against), so they are directly consumable by `capturePosition`. `rect` is the selection's bounding box and `rects` its line boxes (`SelectionRect { x, y, width, height }`, exported from the reader). The host relays through `ContentHostOptions.onSelection`; the validator requires both, every number finite and non-negative, and drops the message otherwise — the frame is the only source and always sends both.

**Geometry — why the frame sends it, and in what space.** The host cannot measure the selection itself: the `Range` lives behind the opaque origin, so this is the same forward-what-the-host-cannot-measure reasoning as `tap`'s `x/y/width/height` and `imagetap`'s `src`. The frame reports the selection's `Range.getClientRects()` — one box per line — plus their union, mirroring the platform's `getClientRects()`/`getBoundingClientRect()` vocabulary (`rects`/`rect`); per-glyph boxes are not reported, a popover needs a line and a bounding box, nothing finer. Numbers are rounded to 2 decimals, and `rect` is computed from the rounded `rects`, so `rect` is *exactly* their union.

- **Coordinate space: frame-viewport px == container padding-box px.** `ContentHost` mounts the iframe as `display:block; border:0; width:100%; height:100%` inside the container, so the frame's viewport origin *is* the container's padding-box origin and the frame translates nothing. A host anchors a popover with `rect` directly inside a `position: relative` wrapper around a border-less reader element, or adds the element's `offsetLeft/Top` + `clientLeft/Top` when it has a border. This is an invariant, proven by a tripwire in `test/browser/selection.browser.mjs` (the iframe's client rect equals the container's padding box) — if the mount style ever gains a border or inset, the frame must subtract it (or the host add it), and that test says so.
- **Clipped to the visible page.** Each line box is intersected with the visible page — the root box in paginated mode (a keyboard-extended selection runs into the off-page columns to the right, whose boxes lie in nowhere), the frame viewport in scrolled mode (the root is the whole document there; scroll is already applied, so `rect` is where the text is *on screen*) — implemented as the intersection of the two, so `x`/`y` are never negative. Boxes left with no area are dropped; if none survive, `rects` is `[]` and `rect` is `{0,0,0,0}` — the event still fires with `text` and `position`, because the selection is real even if nothing of it is visible. Clipping also bounds the payload: a whole-page selection is ~30 boxes, and Cmd+A is bounded to the visible page rather than the section, so no explicit cap is needed.
- **Not on every `selectionchange`.** Geometry rides the same completion-only forward as the text (below), so a popover does not jitter during a drag. Live tracking would be a separate opt-in.
