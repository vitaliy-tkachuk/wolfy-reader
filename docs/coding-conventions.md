...
- **No JSX anywhere under `src/`.** Node's `stripTypeScriptTypes` cannot strip JSX, and `node --test`, the demo server and the pack check's runtime probe all run the sources through it. React code uses `createElement` (see `src/react/index.ts`); `.tsx` files and a `jsx` compiler option are not introduced.
