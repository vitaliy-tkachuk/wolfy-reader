---
name: analyze
description: Use when the user describes new work to be planned OR asks about / refines an existing local task. New work — "analyze X", "look at Y", "we need to add Z", "there's a bug in W", "propose a stack", "recommend how to build this", "design implementation approach". Existing task — "clarify T004", "why does T004 do X", "what's the plan for T012", "tweak the implementation plan for T007", "add an edge case to T009". Also handles initial architecture/stack proposals after describe-project and answers questions about recorded decisions in docs/architecture.md and docs/domains/. Classifies new work into Level 0–3 (plus always-approval triggers) using docs/feature-workflow.md, scaffolds a local task file in docs/tasks/, and refines existing task files in place. Does NOT write implementation code — hands off to the implement skill.
---

# Analyze skill

Owns a task's entire pre-implementation life: turn a free-form request into a task file, then answer questions about it and refine it in place — all before the `implement` skill executes it. There is no approval flag; running `implement` is the go-ahead. Analyze and scaffold only — never write implementation code here.
...
- **Existing-task mode** — the message references an existing task: a `T\d{3}` id, or a topic that maps to a task in `docs/tasks/` ("the auth task"), or it asks to discuss/refine one ("tweak the plan", "add an edge case"). → go to **Branch B**.
...
Print a short receipt: files created, files to update, next action. The task is ready to implement as soon as it is scaffolded — running `implement` is the go-ahead. Example:

```
Created: docs/tasks/T004_add_oauth_callback.md
Next: review it, then say "implement T004" — or "clarify T004 ..." to refine it first.
```

---

# Branch B — Existing task (question / refine)

Once a task exists, this branch handles **questions about it** and **incremental refinements** — without re-scaffolding or implementing anything. There is no approval step; when the user is satisfied, they run `implement` directly.
...
- **Escalation** — proposed change exceeds the task's scope:
  ...
  - implementation requested ("now do it", "apply the change") → hand off to `implement`.
...
Files updated: docs/tasks/T004_add_oauth_callback.md
Next: "implement T004" when ready, or keep refining.
```
