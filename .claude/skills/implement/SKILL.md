...
7. **Knowledge graph** (skip if the repo has no `graphify-out/`): the local `post-commit` hook re-runs AST extraction after every commit (no LLM, no tokens), so never trigger a manual rebuild for ordinary code changes — this step only decides what gets **committed**. Refresh the committed graph when this task changed the repo's *shape*: files or modules added, removed, renamed, or moved, or a new domain opened. Also refresh when the task was doc- or semantics-heavy — run `graphify . --update` first, since the semantic layer and community labels are not auto-refreshed. Skip it for a fix inside existing files. When refreshing, commit `graphify-out/` in its **own** commit (`chore: refresh knowledge graph`) after the task's commit — never staged together with it. Expect `graphify-out/` to show as modified even when you skip; leave it dirty. If `graphify hook status` reports the hooks are missing, say so in the report rather than rebuilding by hand every time.
